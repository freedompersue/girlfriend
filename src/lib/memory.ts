...
可提取的信息类型包括：name(姓名), birthday(生日), age(年龄), occupation(职业), hobby(爱好), location(所在地), pet(宠物), food_preference(饮食偏好), relationship_status(感情状态), recent_event(近期事件), mood(心情状态), study(学业), work_situation(工作情况)。
如果没有可提取的信息，返回空数组 []。
只返回 JSON，不要其他内容。`,
        },
        {
